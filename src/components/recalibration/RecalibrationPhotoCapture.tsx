...
interface RecalibrationPhotoCaptureProps {
  checkpointDay: number;
  onComplete: (photoSetId: string) => void;
}

const RecalibrationPhotoCapture = ({ checkpointDay, onComplete }: RecalibrationPhotoCaptureProps) => {
  const { user } = useAuth();
  ...
  const uploadedRequired = PHOTO_SLOTS.filter((s) => s.required && photos[s.key]).length;
  ...
  const handleUploadAll = async () => {
    ...
    try {
      ...
      for (const [key, file] of Object.entries(photos)) {
        ...
        const path = `${user.id}/day-${checkpointDay}/${key}.${ext}`;
  ...
  return (
    <div className="w-full space-y-6 animate-fade-in">
      <div className="text-center space-y-1">
        ...
        <h2 className="text-lg font-semibold text-foreground">Photo Recalibration</h2>
        <p className="text-sm text-muted-foreground">
          Upload new photos for structural comparison and index recalibration.
        </p>
      </div>
...
export default RecalibrationPhotoCapture;
